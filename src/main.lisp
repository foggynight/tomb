    ...
    (crt:with-screen (scr :cursor-visible nil
      ...
      (crt:with-windows ((note-win
                         ...
                         (depth-win
                          :dimensions '(1 8)
                          :position '(23 72)))
        (let ((window-stack (make-instance 'crt:stack)))
          (dolist (win (list scr note-win game-win stat-win depth-win))
            (crt:stack-push win window-stack))
